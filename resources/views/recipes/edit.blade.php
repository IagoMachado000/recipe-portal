...
<section class="container">
    ...
    <p class="fs-3 text-body-tertiary">altere os detalhes abaixo para editar a receita.</p>

    <form action="{{ route('recipes.update', ['recipe' => $recipe->id]) }}" method="post" novalidate class="row g-4">
        @csrf
        @method('put')
        ...
        <div class="col-12">
            ...
            <textarea
                ...
                class="form-control @error('description') is-invalid @enderror">{{ $recipe->description ?? old('description') }}</textarea>
            @error('description')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
